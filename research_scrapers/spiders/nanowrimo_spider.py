...
class NanoWrimoSpider(CrawlSpider):
    ...
    def parse_posts(self, response):
        ...
        if 'ft' in response.meta:
            ...
            ft['forum_name'] = response.url.split('/')[5]
            ft['url'] = response.url
            ft['responses'] = []
    ...
    def parse_profile(self, response):
        ...
        p = Profile()
        p['author'] = safe_list_get(hxs.select("//h1[@id='profile_name']/text()").extract(), 0).replace('\n','')
        p['age'] = safe_list_get(profile_box.select(".//dt[text()='Age:']/following::dd[1]/text()").extract(), 0)
        p['location'] = safe_list_get(profile_box.select(".//dt[text()='Location:']/following::dd[1]/text()").extract(), 0)
        p['occupation'] = safe_list_get(profile_box.select(".//dt[text()='Occupation:']/following::dd[1]/text()").extract(), 0)
        ...
        participation = ''
        for y in years:
            participation += safe_list_get(y.select(".//img/@title").extract(), 0) + ', '
